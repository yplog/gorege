...
> **Performance note:** When the declared Cartesian product is within the budget, shadow analysis walks it once and uses the engine trie to identify each first-match winner. For a larger product, analysis instead walks each live rule's effective product: exact matchers contribute one value, `AnyOf` contributes its deduplicated declared values, and wildcard or omitted trailing matchers contribute all declared values. Enumeration stops for a rule as soon as it wins. The default shared budget is 100 000 tuples; use `WithAnalysisLimit(n)` to adjust it, or pass a negative value to skip all warning analysis. If the budget runs out, completed dead/shadow decisions are returned first and one `WarningKindAnalysisLimitExceeded` warning identifies the first rule whose shadow status remains unchecked.
